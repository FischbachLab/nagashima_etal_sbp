...
def generate_tree(
    ...
    def layout(node):
        if node.is_leaf():
            if formatted_name_dict:
                N = faces.TextFace(formatted_name_dict[node.name])
            else:
...
def main():
    ...
    if len(sys.argv) == 5:
        formatted_names = sys.argv[4]
    else:
        formatted_names = None

    if formatted_names is not None:
        fomatted_name_dict = dict()
        with open(formatted_names, "r") as names:
            for idx, line in enumerate(names):
                # skip header
                if idx == 0:
                    continue
                genome_name, formatted_genome_name = line.rstrip().split("\t")
                fomatted_name_dict.update({genome_name: formatted_genome_name})
        logging.info(
            f"Tree leaves will be renamed according to the provided formatting"
        )
    else:
        fomatted_name_dict = None

    genomes, color_dict, common_ancestor = parse_summary_file(summary_file, out_prefix)

    generate_tree(
        tree_file, genomes, color_dict, common_ancestor, out_prefix, fomatted_name_dict
    )
