...
undf_fmt_df = read_excel("ForKazuki/1_UndefinedHuman_20200722_FMT1_MIDAS.xlsx") 
...
```{r}
mbfv1_df = read_csv("ForKazuki/mbfv1_dataframe_minRelAbund_0.csv", show_col_types = FALSE)
...
```{r}
mbfv2_df = read_csv("ForKazuki/8_backfill2_mbfv2_dataframe_minRelAbund_0_Sunit_backfill2_MIDAS.csv", show_col_types = FALSE)
...
```{r read_kazuki_data}
kazuki_df = read_excel("221111conventional mouse.xlsx")[1:5]
...
```{r}
midas1_taxonomy_df = read_csv("ForKazuki/midas_genome_taxonomy.csv", show_col_types = FALSE) %>% 
...
```{r}
taxa_colors_list <- c(
  Actinobacteria = "#3953A5",
  Bacteroidetes = "#0D7D3F",
  Firmicutes = "#EE2025",
  Proteobacteria = "#7030A0",
  Verrucomicrobia = "#F68620",
  Other = "#C8C8C8"
  )
```
...
```{r}
taxa_df = left_join(midas1_taxonomy_df, taxa_colors, by = "phylum")
taxa_df 
```
...
```{r}
ggsave(filename = "kazuki_hCom_compare.eps",plot = p, dpi = 300, height = 9, width = 16, units = "cm")
```

```{r}
ggsave(filename = "kazuki_hCom_compare.png",plot = p, dpi = 300, height = 9, width = 16, units = "cm")
```
